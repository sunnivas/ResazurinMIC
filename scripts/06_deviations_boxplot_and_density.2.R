...
# colorvector <- brewer.pal(8,"Paired")[1:8]
...
p <- ggplot(BestEC.p, aes(x=antibiotic, y=deviation)) + geom_boxplot(aes(fill=antibiotic)) +  
  scale_y_continuous(expression("log"[2]*" deviation from Etest"),breaks=c(-10:10 ), lim=c(-8,10)) +
  ...
  geom_text(data=data.frame(), aes( x = names(per), y = rep(-8, 7), label = per), size=6.0, vjust = 0.5, hjust = 0.5)
...
dev_Exp <- dev + theme_bw(base_size = 18)+
  theme(legend.justification=c(-0.01,1.11),
...
cat_agr_s$onedeviation <- round( cat_agr_s$onedeviation * 100, digits = 1 )
cat_agr_s$twodeviation <- round( cat_agr_s$twodeviation * 100, digits = 1)
cat_agr_s$fourdeviation <- round( cat_agr_s$fourdeviation * 100, digits = 1)
# reshape long
